...
def main():
    # 设备：ROCm 下 torch.cuda.is_available() 也是 True
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("device:", device)

    # 数据集
    train_raw = datasets.EMNIST(
        root=DATA_DIR, split="byclass", train=True, download=True, transform=transform
    )
    test_raw = datasets.EMNIST(
    ...
    test_loader = DataLoader(
        test_ds, batch_size=BATCH, shuffle=False, num_workers=2, pin_memory=True
    )
    ...
    best_acc = 0.0
    best_path = OUT_DIR / "tiny36_best_float.pth"
    ...
    export = {
        "meta": {
            "name": "tiny36_cw90_int8_array",
            "classes": 36,
            "labels": "0-9,a-z",
            "input": {"h": 28, "w": 28, "layout": "HW_gray_0_1"},
            "preprocess": "rot90_cw",
            "note": "trained on EMNIST(byclass) digits+lowercase, rotated 90deg clockwise",
        },
        "layers": [
            {
                "type": "conv2d",
    ...
    out_json = OUT_DIR / "tiny36_int8_array.json"
